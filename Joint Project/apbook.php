...
else
{
    ...
    if ($result->num_rows > 0) 
    {
        while(($row = $result->fetch_assoc()) && $isfound == false)
        {
            ...
            {
                $test = openssl_decrypt(hex2bin($row['vac_state']), $cipher, hex2bin($row['randkey']), OPENSSL_RAW_DATA, hex2bin($row['iv']));
                echo $test;
                if($test == "Not Vaccinated")
                {
                    ...
                    if($conflict == false)
                        {
                            ...
                            $cipher_apType = bin2hex(openssl_encrypt("Vaccination", $cipher, $key, OPENSSL_RAW_DATA, $iv));
                            ...
                            if($stmt = mysqli_prepare($dbconnect, $sql))
                            {
                                ...
                                if(mysqli_stmt_execute($stmt))
                                {
                                    ...
                                    $sql = "UPDATE patient SET vac_state = ? WHERE pps_no = ?";
                                    if($stmt = mysqli_prepare($dbconnect, $sql))
                                    {
                                        mysqli_stmt_bind_param($stmt, "ss", $param_teststate, $param_pps_no);
                                        $param_teststate = bin2hex(openssl_encrypt("Vaccination Pending", $cipher, $key, OPENSSL_RAW_DATA, $iv));;
